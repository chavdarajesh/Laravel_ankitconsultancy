...
@section('css')
    <style>
        ...
        .switch_payment_verified,
        .switch_payment_not_verified {
            position: relative;
        ...
        .switch_payment_verified input,
        .switch_payment_not_verified input {
            opacity: 0;
            width: 0;
            height: 0;
        }
        ...
        input:checked+.slider_payment_verified:before,
        input:checked+.slider_payment_not_verified:before {
        ...
        /* Rounded sliders */
        .slider_payment_verified.round,
        .slider_payment_not_verified.round {
            border-radius: 34px;
        }

        .slider_payment_verified.round:before,
        .slider_payment_not_verified.round:before {
            border-radius: 50%;
        }
    </style>
...
@section('content')
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Payment /</span> Not Verified Payment</h4>

        <div class="row">
            <div class="col-md-12">
                <ul class="nav nav-pills flex-column flex-md-row mb-3">
                    <li class="nav-item">
                        <a class="nav-link" href="{{ route('admin.get.first_payment') }}"><i
                ...
                <div class="card">
                    <h5 class="card-header">All Not Verified Payment</h5>
                        ...
                        <table class="table table-hover " id="example">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">Is First</th>
                                    <th class="text-center">Payment SS</th>
                            ...
                            <tbody class="table-border-bottom-0">
                                ...
                                <td class="text-center">
                                    <img src="{{ asset($Payment->payment_screenshot) }}"
                                        alt="{{ asset($Payment->payment_screenshot) }}" width="40px" height="40px"
                                        data-bs-toggle="modal" data-bs-target="#fullscreenModal-{{ $Payment->id }}"
                                ...
                                <td class="text-center"> <input data-id="{{ $Payment->id }}" class="payment_status"
                                        type="checkbox" data-onstyle="success" data-offstyle="danger" data-toggle="toggle"
                                        data-on="Active" data-off="InActive" {{ $Payment->status ? 'checked' : '' }}>
                                </td>
                                <td class="text-center">
                                    ...
                                    <div class="modal fade" id="payment-delete-modal-{{ $Payment->id }}"
                                        ...
                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form
                                                        action="{{ route('admin.delete.not_verified_payment', $Payment->id) }}"
                                                        method="post">
    ...
    <script>
        ...
        $(function() {
            $('.payment_verified').change(function() {
                ...
                $.ajax({
                    type: "POST",
                    url: '{{ route('admin.update.not_verified_payment.is_verified') }}',
                    data: {
                        "_token": "{{ csrf_token() }}",
            ...
            $('.payment_status').change(function() {
                ...
                $.ajax({
                    type: "POST",
                    url: '{{ route('admin.update.not_verified_payment.status') }}',
                    data: {
                        "_token": "{{ csrf_token() }}",
                        'status': status,
                        'id': id
